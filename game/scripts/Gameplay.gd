...
@export var beatmap_path: String = "res://charts/badapple_hex.json"
@export var audio_path:   String = "res://assets/audio/Bad-Apple-Cut-Audio.ogg"
@export var video_path:   String = "res://assets/video/Bad-Apple-Cut-Video.ogv"
...
const RESULT_JACKET: Texture2D = preload("res://assets/images/jacket_badapple.png")
const CHIBI_BEST: Texture2D = preload("res://assets/images/bestresult.png")
const CHIBI_GOOD: Texture2D = preload("res://assets/images/goodresult.png")
const CHIBI_BAD:  Texture2D = preload("res://assets/images/badresult.png")
...
## ── Leaderboard export ──────────────────────────────────────────────────
##
## Every finished run appends one record to scores.json, which the web page in
## leaderboard/ polls.  The file is written in two places: user:// always works
## (including in exported builds), and a "public" copy lands in the leaderboard
## folder itself, next to index.html, so the page can fetch it as a sibling
## without the player hunting through the OS user-data directory.
##
## In the editor that folder is ../leaderboard, beside the Godot project.  In
## an exported build it is a leaderboard folder beside the executable, so
## shipping the page means copying that one directory across.

func _scores_paths() -> Array:
	var paths: Array = ["user://scores.json"]
	var public_dir: String = ""
	if OS.has_feature("editor"):
		public_dir = ProjectSettings.globalize_path("res://").path_join("../leaderboard")
	else:
		public_dir = OS.get_executable_path().get_base_dir().path_join("leaderboard")
	public_dir = public_dir.simplify_path()
	if public_dir != "" and DirAccess.dir_exists_absolute(public_dir):
...
func _save_result() -> void:
	...
	for path in _scores_paths():
		...
		# has no such restriction, so the leaderboard page re-injects this on a
...
const RES_BG_ART: Texture2D = preload("res://assets/images/game_loss.png")
